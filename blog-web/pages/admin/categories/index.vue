<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      ...
      <button @click="showModal = true" class="btn-primary">+ 新建分类</button>
    </div>
      ...
      <table class="w-full">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="cat in categories" :key="cat.id" class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 text-sm text-gray-600">{{ cat._count?.posts || 0 }}</td>
            <td class="px-6 py-4 text-right">
              <div class="flex items-center justify-end gap-2">
                <button @click="editCategory(cat)" class="p-2 text-gray-400 hover:text-blue-600 transition" title="编辑">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                </button>
                <button @click="deleteCategory(cat.id)" class="p-2 text-gray-400 hover:text-red-600 transition" title="删除">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl p-6 w-full max-w-md mx-4">
        <h2 class="text-xl font-bold text-gray-900 mb-4">{{ editing ? '编辑' : '新建' }}分类</h2>
...
const saveCategory = async () => {
  try {
    if (editing.value) {
      await $api(`/categories/${editing.value}`, { method: 'PUT', body: form.value });
    } else {
      await $api('/categories', { method: 'POST', body: form.value });
    }
